...
export class Css {
  ...
  get(colors: string[], options: CssOptions) {
  ...
  private get angle(): string {
    ...
    return options.angle && options.type === 'linear'
      ? `${options.angle}deg, `
      : '';
  }
  ...
  private get extentKeyword(): string {
    ...
    const options = this.options as CssOptions;
    const afterExtent = options.top && options.left
      ? ` at ${options.left}% ${options.top}%, `
      : ', ';
    if (
      options.shape === 'ellipse' &&
      options.extentKeyword &&
      options.extentKeyword !== 'none'
    ) {
